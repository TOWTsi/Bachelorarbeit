...
public final class CheckpointDecision {
	...
	private static boolean getDynamicDecision(RuntimeTask task) {
		...
		if( outInRate <= CheckpointUtils.getLowerBound()){
			...
			task.setWhyCheckpointing("The Out/In Rate is smaller than the Lower Bound (" + 
										outInRate + " <= " + CheckpointUtils.getLowerBound() + ")");
			return true;
		}
		...
		if(userCPU >= 80){
			LOG.info("CREATING CHECKPOINT");
			task.setWhyCheckpointing("The User CPU is " + userCPU);
			return true;
		}
