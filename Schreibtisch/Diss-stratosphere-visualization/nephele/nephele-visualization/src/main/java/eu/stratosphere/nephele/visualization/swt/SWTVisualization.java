...
public class SWTVisualization {
	...
	/**
	 * The directory containing the Nephele configuration.
	 */
	...
	public static void main(String[] args) {
		...
		//set the global CONF_DIR for further use in the API
		CONF_DIR = configDir;
		...
		//Set the global CONFIGURATION for further use in the API
		...
		try {
			...
			//Now the GUI start always, also if there is nothing running on the given port
			//e.printStackTrace();
			rpcService.shutDown();
			LOG.warn("No RPC Service initilized");
			isNepheleOn = false;
			//System.exit(1);
			//return;
		}
		
		FaultToleranceAPI faultToleranceAPI = new FaultToleranceAPI(CONFIGURATION, CONF_DIR);

		final SWTVisualizationGUI swtVisualizationGUI = new SWTVisualizationGUI(jobManager, queryInterval, faultToleranceAPI);
		
		//Checks if Nephele is already started
		swtVisualizationGUI.setNepheleStatus(isNepheleOn);
