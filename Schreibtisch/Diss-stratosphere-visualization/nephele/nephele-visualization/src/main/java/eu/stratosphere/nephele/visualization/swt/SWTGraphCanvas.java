...
public class SWTGraphCanvas extends Canvas implements PaintListener, Listener, MouseMoveListener, MouseListener, Runnable,
	...
	/**
	 * For formating and managing the Text in infoText
	 */
	...
	/**
	 * Sets the SWTInfoBox for further manipulation
	 * @param infoManaging The SWTInfoBox Object from the SWTVisualizationGUI
	 * @author bel
	 */
	public void setInfoManaging(SWTInfoBox infoManaging) {
		this.infoManaging = infoManaging;
	}
	...
	public void mouseMove(MouseEvent arg0) {
		...
		//If mouse is not over any Task or Component, then dispose the ToolTip
		...
		//The Mouse is over a Task or Component
		} else {

			final Point clientPoint = this.toDisplay(arg0.x, arg0.y);
			//If the Component is still the same, only update the position of the component
			if (selectedComponent == toolTipComponent) {
				if (this.displayedToolTip != null) {
					//this.displayedToolTip.move(clientPoint.x, clientPoint.y);
				}
			//If the Component was not shown
			} else {
				// A new component
				// Dispose the last toolTip if it exists
				...
				this.displayedToolTip = selectedComponent.constructToolTip(getShell(), this, clientPoint.x,
					clientPoint.y);
				//Wird statt obiger Zeile direkt auf null gesetzt klappt es nicht. Obige Zeile macht also mehr
				//this.displayedToolTip = null;
				//If the setting was successful move it to the right position
	...
	/**
	 * If the user has clicked on an Task, show information about the Task
	 * in the infoBox.
	 * @author bel
	 */
	@Override
	public void mouseDown(MouseEvent arg0) {
		//System.out.println("TollTip Blalala");
		
		final AbstractSWTComponent selectedComponent = this.swtGraph.getSelectedComponent(arg0.x, arg0.y);
		
		if (selectedComponent != null) {
			ManagementVertex managementVertex = selectedComponent.getManagementVertex();
		
			if (managementVertex != null) {
				this.infoManaging.setVertexInformation(managementVertex);
