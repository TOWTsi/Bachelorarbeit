...
public class JobClient {
	...
	/**
	 * List of all printed events
	 */
	private ArrayList<AbstractEvent> eventList = null;
	...
	public long submitJobAndWait() throws IOException, InterruptedException, JobExecutionException {
		...
		while (true) {
			...
			//für die GUI brauche ich die Events auch außerhalb:
			while (it.hasNext()) {

				final AbstractEvent event = it.next();
				//final AbstractEvent event2 = this.eventIterator.next();
				...
				//für die GUI brauche ich die Events auch außerhalb:
				//Events die hier geprintet werden sollten also in die eventList aufgenommen werden
				this.eventList.add(event);

				//Hier ein Aufruf an die GUI einbauen? So müsste die GUI nicht permanent Anfragen schicken
				//sondern reagiert nur, wenn tatsächlich ein neues Event vorliegt
				System.out.println(event.toString());
				
				//System.out.println(event.getSequenceNumber() + " " + event.getTimestamp());
