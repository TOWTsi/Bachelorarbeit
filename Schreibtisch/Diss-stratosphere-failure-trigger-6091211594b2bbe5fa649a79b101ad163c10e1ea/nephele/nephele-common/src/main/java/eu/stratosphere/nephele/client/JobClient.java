...
public class JobClient {
	...
	public long submitJobAndWait() throws IOException, InterruptedException, JobExecutionException {
		...
		while (true) {
			...
			while (it.hasNext()) {
				...
				//System.out.println(event.getSequenceNumber());
	...
	public void debugJob() {
		
		debug = true;
		Scanner scanner = new Scanner(System.in);
		long time;
		boolean firstRun;
		System.out.println("How many times do you want to restart this specific Job? ");
		int reRuns = -1; 
		while(reRuns<0) {
			reRuns = scanner.nextInt();
			if(reRuns<0) {
				System.out.println("Please choose at least 0 runs!");
			}
		}
		for(int i = 0; i<=reRuns;i++) {
			...
			try {
				...
				long time1 = submitJobAndWaitWithFailures(firstRun);
				System.out.println("The second run: ");
				long time2 = submitJobAndWait();
				System.out.println("The " +(i+1)+ ". run has a time difference of "+ (time1-time2));
	 ...
	 *         thrown if the job has been aborted either by the user or as a result of an error
	 */
	public long submitJobAndWaitWithFailures(boolean firstRun) throws IOException, InterruptedException, JobExecutionException {
		
		final JobSubmissionResult submissionResult = this.jobSubmitClient.submitJobWithFailure(this.jobGraph, firstRun);
		if (submissionResult.getReturnCode() == AbstractJobResult.ReturnCode.ERROR) {
			LOG.error("ERROR: " + submissionResult.getDescription());
			throw new JobExecutionException(submissionResult.getDescription(), false);
		}
		...
		/*int numberOfInputTasks = this.jobSubmitClient.getNumberOfTotalInputSubtasks(this.jobGraph);
		int numberOfInnerTasks = this.jobSubmitClient.getNumberOfTotalInnerSubtasks(this.jobGraph);
		int numberOfOutputTasks = this.jobSubmitClient.getNumberOfTotalOutputSubtasks(this.jobGraph);
		*/
		...
		while (true) {
			...
			while (it.hasNext()) {
				...
				if(event.toString().contains("switched to RUNNING")) {
