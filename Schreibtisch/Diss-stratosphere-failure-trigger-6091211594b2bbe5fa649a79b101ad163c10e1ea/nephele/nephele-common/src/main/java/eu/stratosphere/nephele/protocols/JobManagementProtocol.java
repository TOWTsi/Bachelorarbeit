...
public interface JobManagementProtocol extends RPCProtocol {
	 ...
	 *         thrown if the caller is interrupted while waiting for the response of the remote procedure call
	 */
	JobSubmissionResult submitJobWithFailure(JobGraph job, boolean firstRun) throws IOException, InterruptedException;
	
	void tryToKillTask(String vertexName) throws IOException, InterruptedException;
	/*
	int getNumberOfTotalInputSubtasks(JobGraph job);
	int getNumberOfTotalInnerSubtasks(JobGraph job);
	int getNumberOfTotalOutputSubtasks(JobGraph job);
	String getFileNameOfConfigurations();*/
